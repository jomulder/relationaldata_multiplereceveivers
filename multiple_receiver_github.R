...
####### Look at contribution of different components (fixed effects, random popularity effects, latent effects)
...
lines(recpref,col=2)
#only look at most popular receivers for given sender
welkpop <- order(recpref,decreasing=TRUE)[1:15]
plot(1:length(welkpop),recpref[welkpop],"l")
for(rep in 1:nrow(out1$beta)){
  lines(out1$Yrep[[welk]][rep,welkpop],col="grey")
}
lines(recpref[welkpop],col=2)
#ppc for frequencies of number of receivers
to <- table(apply(do.call(rbind,Y),1,sum,na.rm=TRUE))
plot(as.integer(names(to)),to/sum(to),col=1,lwd=3,xlim=c(0,20))
for(rep in 1:nrow(out1$m)){
  tm <- table(out1$m[rep,])
  lines(as.integer(names(tm)),tm/sum(tm),col=2)
}
points(as.integer(names(to)),to/sum(to),col=1,lwd=3)
